...
\item During the import, the extension is removed, and the name is lowercased, to be used as the \textbit{\$filename} prefix for all tables and files created during the autogloss or import process.  With the test file, the \textit{\$filename} is \textbf{mytest}.  With an original file named Patagonia1, the \textit{\$filename} would be \textbf{patagonia1}.
