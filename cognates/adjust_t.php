...
while ($row0=pg_fetch_object($result0))
{
	...
	$loc[2]=$loc[0]+1; // following location
	...
	/*
	// Rules handling the previous slot.
	...
	*/
	...
	// Rules handling the following slot.
	$sql2=query("select * from $words where utterance_id=$utt and location=$loc[2]");
	//$result2=pg_query($db_handle,$sql2) or die("Can't get the items");
	while ($row2=pg_fetch_object($sql2))
	{
		$s2=$row2->surface; // following surface
		$a2=$row2->auto; // following auto
		
		// fan<man
		if (preg_match("/^fan$/", $s0) && preg_match("/^(yno|yna|hyn|hynny)$/", $s2))
		{
			$d1_da=query("update $words set cognate=cognate||'+d1' where utterance_id=$utt and location=$loc[0]");
			echo "Deleting $utt,$loc[0]\n";
		}
		
		// pan (conj)
		if (preg_match("/^pan$/", $s0) && preg_match("/\.V\.(?!INFIN)/", $a2))
		{
			$d1_da=query("update $words set cognate=cognate||'+d1' where utterance_id=$utt and location=$loc[0]");
			echo "Deleting $utt,$loc[0]\n";
		}
	...
	$prev_utt=$utt;
	$prev_l=$loc[0];  // location from previous loop
	$prev_a=$a0;  // auto from previous loop
