...
$fp = fopen("cypriot/outputs/imported_split/".$filename."_split.txt", "w") or die("Can't create the file");  // Corrected split.
//$fp = fopen("cypriot/outputs/remsplit/".$filename."_split.txt", "w") or die("Can't create the file");  // Initial split.

$sql1="select utterance_id, speaker from $utterances where surface !~ 'www' order by utterance_id";  // List the utterance_ids and speaker in sequence.
...
while ($row1=pg_fetch_object($result1))
{
	...
	{
		if(empty($locarray[1]))  // Handle the end of the utterance:  if there is no $locarray[1] ...
		{
			$myand="";  // ... we are at the last clause-marker, so we don't need an upper bound ...
		}
		else  // ... and if there is a $locarray[1] ...
		{
			$myand=" and location<$locarray[1] ";  // ... we need to use it as the upper bound.
		}
		...
		{
			$clause.=$row4->surface." ";
			//$clause.=$row4->surface."@".$row4->langid." ";
			$auto.=$row4->auto." ";
		...
		fwrite($fp, $clause."\n");  // Write out the split clauses, without the speaker id.
