...
$fp = fopen("cognates/$filename/".$filename."_t_by_length_ext.txt", "w") or die("Can't create the file");  // Open a file for the results.
...
for ($i=0; $i<=$maxno; $i++)  // Loop through the clauses of each length.
{
	...
	$sql_st=query("select count(*) from ".$filename."_cognates where maxloc-minloc=$i and external='ST'");
	...
	$sql_nst=query("select count(*) from ".$filename."_cognates where maxloc-minloc=$i and external='NST'");
	...
	$sql_snt=query("select count(*) from ".$filename."_cognates where maxloc-minloc=$i and external='SNT'");
	...
	$sql_nsnt=query("select count(*) from ".$filename."_cognates where maxloc-minloc=$i and external='NSNT'");
